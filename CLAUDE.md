...
**one NFS export** — `192.168.50.32:/mnt/media` (qBittorrent on boopurnoes bind-mounts it at `/data`;
...
2. For real verification, run the built server and exercise it — the host can reach the Jellyfin
   container directly (e.g. `JELLYFIN_URL=http://172.18.0.2:8096`), and the host-built
